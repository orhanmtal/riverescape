// River Escape - Ses Motoru (Audio Engine) - v1.99.61.119 (AUDIO GLOBAL SYNC)
...
function bgMusicScheduler() {
    ...
    if (!isPaused && !window.isAdShowing && window.audioCtx) {
        // nextNoteTime geride kaldıysa (pause sonrası), şimdiden devam et
        if (nextNoteTime < window.audioCtx.currentTime) {
...
function playPowerupSound() {
    ...
    osc.frequency.linearRampToValueAtTime(800, window.audioCtx.currentTime + 0.1); 
    osc.frequency.linearRampToValueAtTime(1600, window.audioCtx.currentTime + 0.2); 
    gainNode.gain.setValueAtTime(0.5 * isSFXVolume, window.audioCtx.currentTime);
    gainNode.gain.linearRampToValueAtTime(0.01, window.audioCtx.currentTime + 0.4);
    osc.connect(gainNode); gainNode.connect(window.audioCtx.destination);
...
function playSpinClick() {
    ...
    const gain = window.audioCtx.createGain();
    osc.type = 'triangle';
    osc.frequency.setValueAtTime(150, window.audioCtx.currentTime);
    gain.gain.setValueAtTime(0.15 * isSFXVolume, window.audioCtx.currentTime);
...
function playWhooshSound() {
    ...
    const buffer = window.audioCtx.createBuffer(1, bSize, window.audioCtx.sampleRate);
    ...
    noise.connect(f); f.connect(g); g.connect(window.audioCtx.destination);
...
function playUIClick() {
    ...
    osc.frequency.exponentialRampToValueAtTime(800, window.audioCtx.currentTime + 0.05);
    g.gain.setValueAtTime(0.12 * isSFXVolume, window.audioCtx.currentTime);
